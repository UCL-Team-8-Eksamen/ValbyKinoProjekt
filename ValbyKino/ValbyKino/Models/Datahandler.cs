    ...
    public class Datahandler
    {
        ...
        public void PrintMovies(ObservableCollection<Movie> movies)
        {
            ...
            for (int i = 0; i < movies.Count; i++)
            {
                ...
                sw.WriteLine($"{movies[i].OriginalTitle}, {movies[i].LocalTitle}, {movies[i].DirectorFirstName}, {movies[i].DirectorLastName}, {movies[i].OriginalCountry}, {movies[i].NationalReleaseDate}, {shows[1].Date}, {shows[1].Version.ToString()}, {shows[1].ScreeningFormat.ToString()}, , {shows[1].Movie.AlternativeContent}, (nb weeks), {shows.Count}, {admissions}, {shows[1].Admissions * shows[1].Price}, {shows[1].YA}");
            }
            sw.Close();
